...
describe('MetricsPanel', () => {
  ...
  it('renders a section for each slice', () => {
    const m = makeMetrics({
      train: makeSlice({ slice_type: 'train' }),
      test: makeSlice({ slice_type: 'test' }),
    })
    render(<MetricsPanel metrics={m} />)
    expect(screen.getByRole('region', { name: /train slice/i })).toBeInTheDocument()
    expect(screen.getByRole('region', { name: /test slice/i })).toBeInTheDocument()
  })
  ...
  it('renders all three canonical slices in order (train → validation → test)', () => {
    const m = makeMetrics({
      test: makeSlice({ slice_type: 'test' }),
      train: makeSlice({ slice_type: 'train' }),
      validation: makeSlice({ slice_type: 'validation' }),
    })
    render(<MetricsPanel metrics={m} />)
    const headers = screen.getAllByRole('heading', { level: 3 })
    const names = headers.map((h) => h.textContent?.toLowerCase())
    expect(names[0]).toContain('train')
    expect(names[1]).toContain('validation')
    expect(names[2]).toContain('test')
  })
})
